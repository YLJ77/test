...
// Set Adobe Photoshop CS6 to use pixels
...
function capableProcessed(num){
	if (num % 76 != 0 && num % 124 != 0) return false;
	return true;
}

height = resourceHeight % 76 == 0 ? 76 : 124;
...
var dialogue="dialog{\
	...
	group:Group{\
		...
		corrdination:Panel{\
            ...
            x:Group{\
				orientation: 'row', \
                s: StaticText { text:'宽:' }, \
                e: EditText { preferredSize: [50, 20],text:"+resourceWidth+"} ,\
                p: StaticText { text:'px' }, \
            }, \
            y: Group {\
				orientation: 'row', \
                s: StaticText { text:'高:' }, \
                e: EditText { preferredSize: [50, 20],text:"+height+"}, \
                p: StaticText { text:'px' }, \
            } ,\
        }, \
		fileResource:Group{\
			...
			inputText: EditText{ text:'C:/Users/linji/Desktop/批量生成web视图/1.jpg', preferredSize: [180, 20] },\
		},\
	},\
	saveFileType:Group{\
		orientation:'row', \
	...
	saveAsOptions:Panel{\
		...
		saveAsFolder:Group{\
			...
			inputText: EditText  { text:'C:/Users/linji/Desktop/批量生成web视图/create', preferredSize: [180, 20] },\
		},\
	},\
	tips:Panel{\
	    orientation:'row',\
...
//初始窗口对象	
if(capableProcessed(resourceHeight)) {
	var win=new Window(dialogue);

//初始保存设置
//only COMPUSERVEGIF,JPEG, PNG-8, PNG-24, and BMP are supported.
